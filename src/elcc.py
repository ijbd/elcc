...
def get_powGen(solar_cf_file, wind_cf_file):
    ...
    solarPowGen = nc4.Dataset(solar_cf_file)
    windPowGen = nc4.Dataset(wind_cf_file) #assume solar and wind cover same geographic region
...
def get_temperature_data(temperature_file):
    ...
    temperature_data = np.array(nc4.Dataset(temperature_file)["T2M"][:][:][:]).T
...
def main(args):
    ...
    data_sources = load_data_sources(args.data_directory)
    solar_netcdf = load_netcdf(data_sources.solar)
    wind_netcdf = load_netcdf(data_sources.wind)
    hourly_demand = load_hourly_demand(data_sources.demand, args.year, args.regions)
    hourly_temperature = load_hourly_temperature(data_sources.temperature)
    hourly_efor = load_temp_dependent_efor(data_sources.temp_dependent_efor, args.use_temperature_dependent_efor)
    hourly_interchange = load_hourly_interchange(
        args.year, 
        args.regions, 
        args.data_directory
    )
    conventional_fleet = load_conventional_fleet(
        args.data_directory,
        args.regions,
        args.year
    )
    solar_fleet = load_solar_fleet(
        args.data_directory,
        args.regions,
        args.year
    )
    wind_fleet = load_wind_fleet(
        args.data_directory,
        args.regions,
        args.year
    )
    storage_fleet = load_storage_fleet(
        args.data_directory,
        args.regions,
        args.year,
        args.storage_efficiency,
        args.storage_dispatch_strategy
    )
    
    # simulate hourly fleet capacity
    log.info("Simulating fleet dispatch.")
    hourly_fleet_capacity = simulate_conventional_capacity(
        conventional_fleet,
        args.mc_iterations
    ) + simulate_renewable_capacity(
        solar_fleet,
        args.mc_iterations
    ) + simulate_renewable_capacity(
        wind_fleet,
    ...
    log.info("Simulating fleet storage dispatch.")
    hourly_storage_capacity = simulate_storage_capacity(
        storage_fleet,
        hourly_demand, 
        hourly_solar_capacity, 
        hourly_wind_capacity, 
        hourly_conventional_capacity,
        args.mc_iterations
    )

    # build generator for ELCC
    log.info("Building generator.")
    added_generator = build_generator(
        args.added_generator_type,
        args.added_generator_capacity,
        args.added_generator_latitude,
        args.added_generator_longitude
    )

    # get hourly capacity 
    log.info("Simulating added generator dispatch")
    hourly_added_capacity = simulate_renewable_capacity(
        added_generator,
        args.mc_iterations
    )

    # calculate elcc
    log.info("Calculating ELCC.")
    elcc = calculate_elcc(
        hourly_demand
        hourly_fleet_capacity,
        hourly_added_capacity,
        storage_fleet,
        args.mc_iterations,
        added_generator.capacity
    )
    log.info("Ending program.")

if __name__ == "__main__":
    # arguments
    # TODO: make sub-parsers for dependencies
    ...
    parser.add_argument("--target-reliability", type=float, default=2.4)
    parser.add_argument("--output-directory", type=str, 
        default=os.path.join(os.path.dirname(__file__),"output")
    )
    parser.add_argument("--data-directory", type=str, required=True)
    parser.add_argument("--cache-meta-data", action="store_true")
    parser.add_argument("--meta-data-directory", type=str, ) # TODO make dependent on cache-meta-data
    parser.add_argument("--default-efor", type=float, default=.05)
    parser.add_argument("--use-temperature-dependent-efor", action="store_true")
    parser.add_argument("--storage-dispatch-strategy", type=str, choices=["reliability"], required=True)
    parser.add_argument("--storage-efficieny", type=float, default=.8)
    parser.add_argument("--debug", action="store_true")
    parser.add_argument("--added-generator-type", type=str, choices=["solar","wind"])
    parser.add_argument("--added-generator-capacity", type=float, required=True)
    parser.add_argument("--added-generator-latitude", type=float, required=True)
    parser.add_argument("--added-generator-longitude", type=float, required=True)
    ...
    logging.basicConfig(
        ...
        encoding="utf-8",
        level=logging.DEBUG,
        filename=os.path.join(args.output_directory, "log")        
    )
