...
def main(minLon, maxLon,minLat, maxLat, implementStateBounds = False,stateRegionList = [],fileName = "generic_state_bounds"):
    ...
    `implementStateBounds` (bool): set to false, but if specified as true, will read states bounds from folder (stateNetcdfs)
    and uses respective netCDF files to generate bounds

    `stateRegionLis` (list): set to empty on default, but if provided, will read the bounds from those specific states for visualization
    
    `fileName` (str): file path to state boundary file (will be unique to states used)
    """
    #setting up values for MERRA data latitude goes from min to max with .5 spacing while longitude is .625 spacing
    latitudeRange =  np.arange(minLat, maxLat+1,1)
    longitudeRange =  np.arange(minLon,maxLon+1.25,1.25)
    ...
    print("Wrote bounds data for states: %s to: %s" % (stateRegionList,filePath))